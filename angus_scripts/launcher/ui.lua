-- Custom glass overlay for the Cmd+Space launcher.
-- hs.chooser cannot do rounded corners or vibrancy, so this is a webview.
...
local function place(self)
    if not self.webview or not self.webview.frame then
        return
    end
...
function M.create(callbacks)
    ...
    if wv.navigationCallback then
        wv:navigationCallback(function(action)
            if action == "didFinishNavigation" or action == "didFinish" then
                ...
                if overlay.webview and overlay.webview.evaluateJavaScript then
    ...
    overlay.webview = wv
    overlay.pageReady = false
    overlay.pending = nil

    function overlay:show(state)
        place(self)
        -- Paint first. Activate after so Hammerspoon focus cost is not on the open path.
        self.webview:show()
